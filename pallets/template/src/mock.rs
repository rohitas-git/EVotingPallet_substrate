...
// `frame_support::construct_runtime!` is a macro that generates the necessary code to define a runtime
// for a Substrate-based blockchain. 
// It takes in several arguments, including the name of the runtime
// (`Test` in this case), the types of blocks to use (`Block` and `NodeBlock`), and the types of
// extrinsics to use (`UncheckedExtrinsic`). It also specifies which pallets to include in the runtime,
// in this case `System` and `TemplateModule`. The generated code includes various implementations of
// traits required by the Substrate framework, such as `frame_system::Config` and
...
pub fn configure_election_start_and_end_time(root: Runtime, start: BlockNumber, end: BlockNumber)-> DispatchResult{
